    ...
    <el-form :model="value" ref="productAttrForm" label-width="120px" class="form-inner-container" size="small">
      ...
      <el-form-item label="商品规格：">
        ...
        <el-table style="width: 100%;margin-top: 20px" :data="value.skuStockList" border>
          ...
          <el-table-column label="销售价格" align="center" label-class-name="required">
            <template slot-scope="scope">
              <el-input v-model="scope.row.price" clearable></el-input>
            </template>
          </el-table-column>
          <el-table-column label="促销价格" align="center" label-class-name="required">
            <template slot-scope="scope">
              <el-input v-model="scope.row.promotionPrice" clearable></el-input>
            </template>
          </el-table-column>
          <el-table-column label="协议价" align="center" label-class-name="required">
            <template slot-scope="scope">
              <el-input v-model="scope.row.costPrice" clearable></el-input>
            </template>
          </el-table-column>

          <el-table-column label="商品库存" align="center" label-class-name="required">
            <template slot-scope="scope">
              <el-input v-model="scope.row.stock" clearable></el-input>
            </template>
          </el-table-column>
          <el-table-column label="库存预警值" align="center" label-class-name="required">
            <template slot-scope="scope">
              <el-input v-model="scope.row.lowStock" clearable></el-input>
            </template>
          </el-table-column>
          <el-table-column label="SKU编号" width="180" align="center" label-class-name="required">
            <template slot-scope="scope">
      ...
      <el-form-item label="属性图片：" v-if="hasAttrPic">
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductAttrPics">
            <span>{{item.name}}:</span>
            <single-upload v-model="item.pic" type="1" style="width: 300px;display: inline-block;margin-left: 10px"></single-upload>
          </div>
        </el-card>
      </el-form-item>
      ...
      <el-form-item label="商品相册：">
        <multi-upload v-model="selectProductPics" type="1"></multi-upload>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      mergeProductAttrValue() {
        ...
        // 新增加的逻辑，自动选择的按照下面的合并属性
        for (let i = 0; i < this.selectProductAttr.length; i++) {
          let attr = this.selectProductAttr[i];
          if (attr.handAddStatus === 0 && attr.values != null && attr.values.length > 0) {
            this.value.productAttributeValueList.push({
              productAttributeId: attr.id,
      ...
      handleNext() {
        for (let i = 0; i < this.value.skuStockList.length; i++) {
          ...
          if(!this.value.skuStockList[i].promotionPrice){
            //
            this.$message({
              type: 'error',
              message: '请填写促销价格!'
          ...
          if(!this.value.skuStockList[i].costPrice){
            //
            this.$message({
              type: 'error',
              message: '请填写协议价格!'
          ...
          if(!this.value.skuStockList[i].skuCode){
            //
            this.$message({
              type: 'error',
              message: '请填写sku编号!'
...
<style scoped>
  ...
  /deep/ .el-table th.required>div::after {
    content: '*';
    color: #ff4d51;
